...
module.exports.checkUsername = async function ( username ) {
  ...
  Collation only works on find for now, may be updated in future drivers.
  **/
  const userExists = await db.collection( 'users' ).findOne( { 'usernameLower': username.toLowerCase() }, { 'projection': { '_id': 1 } } );
